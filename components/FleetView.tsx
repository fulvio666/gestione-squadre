...
export const FleetView: React.FC = () => {
  const { vehicles, addVehicle, deleteVehicle, importVehicles } = useAppContext();
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Parco Mezzi");
    XLSX.writeFile(workbook, "parco_mezzi.xlsx");
  };
  ...
  const handleFileImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (newVehicles.length > 0 && window.confirm(`Sei sicuro di voler sovrascrivere il parco mezzi esistente con i ${newVehicles.length} record dal file? Questa azione è irreversibile.`)) {
          importVehicles(newVehicles);
          alert(`Dati importati con successo! Aggiunti/aggiornati ${newVehicles.length} mezzi.`);
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div className="md:col-span-1">
        <div className="bg-white rounded-lg shadow p-6 sticky top-8">
          <h3 className="text-lg font-bold text-slate-700 mb-4">Gestisci Mezzo</h3>
          <form onSubmit={handleAddVehicle} className="mb-6">
            <label htmlFor="vehicle-name" className="block text-sm font-medium text-slate-600">Nome Mezzo</label>
            <input
              id="vehicle-name"
              type="text"
              value={newVehicleName}
              onChange={(e) => setNewVehicleName(e.target.value)}
              placeholder="Es. Furgone, Escavatore"
      ...
      <div className="md:col-span-2">
        <h2 className="text-xl font-bold text-slate-700 mb-4">Elenco Parco Mezzi</h2>
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <ul className="divide-y divide-slate-200">
            {vehicles.length > 0 ? vehicles.map(vehicle => (
              <li key={vehicle.id} className="p-4 flex items-center justify-between hover:bg-slate-50">
            ...
            )) : <p className="p-4 text-center text-slate-500">Nessun mezzo presente.</p>}
          </ul>
        </div>
      </div>
    </div>
